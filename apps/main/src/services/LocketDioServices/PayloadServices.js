...
export const createRequestPayloadV4 = async (selectedFile, previewType) => {
  try {
    ...
    if (overlayData?.type === "review" && overlayData?.overlay_id?.startsWith("review:")) {
      const parts = overlayData.overlay_id.split(":");
      const realRating = parseFloat(parts[1]) || 5;
      const maxStars = parseInt(parts[2]) || 5;

      if (maxStars > 5) {
        optionsDataObj.overlays = [
          {
            type: "review",
            text: overlayData.text,
            caption: overlayData.caption,
            overlay_id: overlayData.overlay_id,
            payload: { rating: Math.min(realRating, 5), comment: overlayData.payload?.comment || " " },
          },
          {
            type: "review",
            text: " ",
            caption: " ",
            overlay_id: `review_row2`,
            payload: { rating: Math.max(0, realRating - 5), comment: " " },
          },
        ];
        // Xoá các thuộc tính gốc để server Locket nhận overlays array
        delete optionsDataObj.type;
        delete optionsDataObj.payload;
        delete optionsDataObj.text;
        delete optionsDataObj.caption;
        delete optionsDataObj.overlay_id;
      }
    }
